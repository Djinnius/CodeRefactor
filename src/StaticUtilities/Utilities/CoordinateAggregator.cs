...
/// <summary>
///     Contains methods to aggregate multiple localised coordinates into a single mean coordinate.
/// </summary>
public static class CoordinateAggregator
{
    private static readonly double earthRadiusInKm = 6371; // Earth's radius in km

    public static Coordinate GetCurrentCoordinate(double _currentAngleInRadians, double centreLatitudeInDegrees, double centreLongitudeInDegrees)
    {
        // Read three coordinates from 'instrumentation system' e.g. aeroplanes have multiple sensors in case of a fault.
        var coordinate1 = CoordinateProvider.GetSensorCoordinate1(_currentAngleInRadians, centreLatitudeInDegrees, centreLongitudeInDegrees);
        var coordinate2 = CoordinateProvider.GetSensorCoordinate2(_currentAngleInRadians, centreLatitudeInDegrees, centreLongitudeInDegrees);
        var coordinate3 = CoordinateProvider.GetSensorCoordinate3(_currentAngleInRadians, centreLatitudeInDegrees, centreLongitudeInDegrees);
