...
public class SpeedLimitCalculator
{
    ...
    private double _currentAngleInRadians = 0; // Current angle in radians
    ...
    private readonly CoordinateAggregator _coordinateAggregator;

    public SpeedLimitCalculator(CoordinateAggregator coordinateAggregator)
    {
        _coordinateAggregator = coordinateAggregator; // or new CoordinateAggregator(new CoordinateProvider(new CachingService()));
    }
    ...
    public OneOf<double, None> GetSpeedLimitForCurrentPosition()
    {
        ...
        var currentCoordinate = _coordinateAggregator.GetCurrentCoordinate(_currentAngleInRadians, _centreLatitudeInDegrees, _centreLongitudeInDegrees);
